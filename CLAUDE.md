...
핵심 목적은 "동일한 기능을 **두 가지 도구 연동 방식**으로 구현하고 비교하는 것"입니다.
...
| LLM 모델 | gemma3 | **qwen3.6:27b** (tools/vision/thinking) |
| OS | Ubuntu 20.04 | **Windows 11 네이티브** |
| 도구 연동 | LLM 파싱 후 스크립트가 호출 | **Native tool calling → MCP** 2단계 |
| 대상 사이트 | 실제 코레일 | **로컬 mock 사이트(기본)** + 실제 코레일(opt-in) |
| 자동화 범위 | 예매 진행 | **예약(좌석 확보)까지. 결제·발권은 사람** |
